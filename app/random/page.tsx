...
function getSettings() {
  ...
  const minimumShift = parseInt(minimumShiftStr.replace('°', ''))
...
function generateRandomShifts() {
  ...
  for (let i = 0; i < numShifts; i++) {
    ...
    const endDay = sameDay ? '' : (isToday ? days[Math.floor(Math.random() * days.length)] : days[Math.floor(Math.random() * days.length)])
    ...
    return (
      ...
      >
        <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
          ...
          <button onClick={handleRegenerate} className="px-3 py-1 bg-blue-600 text-white text-xs rounded whitespace-nowrap">
            Random
          </button>
        </div>
        ...
        >
          <div 
            className="flex flex-col items-start gap-10 w-full max-w-[342px] py-3 flex-1"
          >
